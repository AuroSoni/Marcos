/*
Changes Made:
1. Changed MPU9250 WHOAMI_DEFAULT_VALUE from 0x71 to 0x68.
2. -> . and w to W in readByte();
...
class MPU9250_
{
    ...
    float magnetic_declination = -0.10; // Japan, 24th June

public:

    MPU9250_() : aRes(getAres()), gRes(getGres()), mRes(getMres()) {}

	//This setup() checks if MPU and AK are connected by reading a byte from specific addresses
    void setup(WireType& w = Wire)
    {
		//Serial.print("Setup fn started\n");
		delay(1000);
        wire = &w;
		//Serial.print("Wire reference declared\n");

        uint8_t m_whoami = 0x00;
        uint8_t a_whoami = 0x00;

        m_whoami = isConnectedMPU9250();                        //reads a byte from sensor and checks it against MPU9250_WHOAMI_DEFAULT_VALUE and returns 1 if true
        if (m_whoami)
        {
            ...
            a_whoami = isConnectedAK8963();                    //same as MPU9250 but at a different address.
            if (a_whoami)                                    
            {
	...
	//Check if MPU is connected by reading a byte from the device address
    bool isConnectedMPU9250()
    {
		//Serial.print("is connected MPU9250()\n");
		delay(1000);
		//Serial.print("MPU9250_ADDRESS: ");
		//Serial.println(MPU9250_ADDRESS);
		//Serial.print("MPU_9250_WHOAMI_DEFAULT_VALUE : ");
		//Serial.println(MPU9250_WHOAMI_DEFAULT_VALUE);
		//Serial.print("WHO_AM_I_MPU9250: ");
		//Serial.println(WHO_AM_I_MPU9250);
		delay(1000);
        byte c = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);                  //MPU9250 address defined as 0x68 when AD0=0 
        //Serial.print("MPU9250 WHO AM I = ");
        //Serial.println(c, HEX);
        return (c == MPU9250_WHOAMI_DEFAULT_VALUE);
    }

	//Check if AK is connected by reading a byte from the device address
	...
	//To check if byte read is available from MPU
    bool available()
    {
        return (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01);
    }

	//if byte is availale update the acc, gyro and mag.
    void update()
    {
        if (available())
